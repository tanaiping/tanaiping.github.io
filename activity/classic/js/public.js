...
var baseUrl = 'http://activity.peanut8.com'//'http://activity.peanut8.com';

// 分享数据
var shareData = {
  url: baseUrl + "/act/2020/classic/index.html",
  imgUrl: baseUrl + "/act/2020/classic/images/share.png",
  title: "一起康康花生伙伴们的花样生活",
  text: "花样经精彩视频全收录"
};
// 后台接口
var urlConfig = {
  'send': baseUrl + '/20/metro/send',
  'login': baseUrl + '/20/metro/login',
}
...
// 点击获取验证码操作
$(document).on('click', '#Noteh',function () {
  const obj = this
...
// 登录
function login() {
  let  nickName = $("#Name").val().trim();
  let tel = $("#Tel").val();
  let code = $("#verification").val();
  if (nickName == "") {
    nfyg.otherCommon.promptShow("用户昵称不能为空！");
    return false;
  }
  nfyg.otherCommon.getDatanormal("post", urlConfig.login, "json", {
    ...
    if (res.code == 1) {
      // 处理登录成功代码
	  $.cookie('tel', res.data.userId, { expires: 7 });
	  $.cookie('nickName', res.data.nickName, { expires: 7 });
	  window.location.href='../page/index.html?mobile='+res.data.userId +"&nickName="+res.data.nickName;
    } else {
      nfyg.otherCommon.promptShow(res.msg);
    }
  })
