...
commonUtils.showPop2 = function(text,time,fontSize) {
    ...
    if(pop.length > 0){
        ...
        pop = $('<div class="popInfo2" id="popInfo2" style="position: fixed;top: 40%;left: 16%;width:68%;line-height: 50px;height: 50px; background-color: rgba(0, 0, 0, 0.7);z-index: 8899;color: #F7F9EE;text-align: center; font-size: '+fontSize+'rem; border-radius: 10px;">'+ text +'</div>').appendTo('body');
        window.showPopTimer = setTimeout(
            function(){
                pop.hide();
            }, time);
