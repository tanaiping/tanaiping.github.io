...
var clss = ['ballImg ballImg-one', 'ballImg ballImg-two', 'ballImg ballImg-three', 'ballImg ballImg-four', 'ballImg ballImg-five', 'ballImg ballImg-sex', 'ballImg ballImg-seven', 'ballImg ballImg-eight', 'ballImg ballImg-nine', 'ballImg ballImg-ten', 'ballImg ballImg-eleven', 'ballImg ballImg-twelve', 'ballImg ballImg-thirteen', 'ballImg ballImg-fourteen'];
...
window.onload = function() {
	var sc = null;
	var ballPoolImg = document.getElementById("ball-poolImg");
	var screen = document.getElementById("screen");
	var screen2 = document.getElementById("screen2");
	/*document.getElementById("inner").innerHTML='';
	    sc=new Screen('inner',{ballsnum:10, spring:1.3, bounce:-0.9, gravity:0.01});
	    sc.initialize();*/
	getFlag('start').onclick = function() {
		...
		if(count>0){
			if(!$(this).hasClass('disabled')){
				...
				//ballPoolImg.style.display = 'none';
				$(".machine .triangle").addClass("pillar");
				$("#ballOut-img").hide();
				$(".start-bt").attr("id","");
				document.getElementById("inner").innerHTML = '';
				sc = new Screen('inner', {
				...
				timeOut(sc);
				if(count==1){
					$(".play-bt #start").css("opacity",0.8);
				}else{
					$(".play-bt #start").css("opacity",1);
				}
				$(this).addClass('disabled');
				setTimeout(function(){$("#play-bt .start-bt").removeClass('disabled');},2800);
			}
		}else{
			handle(0);
		}
	...
	function timeOut(sc) {
		setTimeout(function() {
			...
			$(".top-demo div").each(function() {
		        $(this).circulate({
		            speed: Math.floor(Math.random()*300) + 10,
			...
			$(".machine .triangle").removeClass("pillar");
			//ballPoolImg.style.display = 'block';
			$(".start-bt").attr("id","start");
			setTimeout(function(){
				$("#ballOut-img").show();
			},800);
			handle(1000);
   ...
   function handle(time){
		 ...
		 console.log(xxx)
		if(xxx){
			$(".prize-word").html("<p>今日挑战次数已用完，</p><p>明天再来尝试吧！</p>");
			$("#prize-numWrap").show();
		} else {
