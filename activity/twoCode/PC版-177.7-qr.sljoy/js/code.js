$(function(){
	...
	sys_getwebuinfo(utoken);
	$(".tx-img").click(function(){
		var istomb = parseInt($(this).attr("istomb"));
		if(istomb == 1){
			window.location.href = 'http://qr.sljoy.cn/markPark/Managers/main.html'
		}
	})
	...
	var inters = 0;
	var wcode = '';
	var utoken = localStorage.getItem('utoken') == null?'':localStorage.getItem('utoken');
	//获取登录码
	...
	function sys_checklogincode(){
		$.ajax({
		  ...
		  contentType: "application/json",
		  dataType: 'json',
		  data:JSON.stringify({'wcode':wcode}),
		  beforeSend: function (xhr) {
		     xhr.setRequestHeader('utoken', utoken);
		  },
			success: function (data) {
				...
				if(data.code==100){
					...
					inters = 0;
					$(".blackbg,.login-pop").hide();
					sys_getwebuinfo(data.data.utoken);
					localStorage.setItem('utoken',data.data.utoken);
				}else if(data.code == -1){
					showMsg(data.codemsg);
				}else{
					inters++;
	...
	//获取用户信息-是否登录
	function sys_getwebuinfo(utoken){
		$.ajax({
		  type: "post",
		  ...
		  beforeSend: function (xhr) {
			...
			success: function (data) {
				...
				if(data.code == 100){
					...
					$(".acc-name").text(data.data.nickname+"（"+data.data.userid+"）")
					$(".logined").show();
					$(".tx-img").attr("istomb",data.data.rightinfo.istomb);
				}else{
					//showMsg(data.codemsg);
				}
			}
