//JavaScript代码区域
  layui.use(['jquery','element','layer', 'form','laypage',"laydate"], function(){
    ...
    $(".verifi_detail").click(function(){
    	$("#iframe",parent.document.body).attr({"src":"shop/data/verifiData.html?rdm="+Math.random(),"couponId":couponId});
    })
    ...
    $(document).on('click','.ticket-detail',function(){
    	...
    	$("#iframe",parent.document.body).attr({"src":"shop/data/verifiData_detail.html?type_detail=1&rdm="+Math.random(),"couponId":couponId});
    });
    ...
    function getDetailData(couponId,curPage,initPage){
    	$.ajax({
	        ...
	        data:JSON.stringify({"couponId":couponId,"pageNo":curPage,"bisId":userId}),
	        contentType: "application/json",
	        dataType: 'json',
	        beforeSend: function (xhr) {
				 xhr.setRequestHeader('token', token);
	        	...
	        	if(data.resultCode==0){
	        		...
	        		if(res.data!=null){
	        			...
	        			for(var i=0;i<res.data.length;i++){
		        			...
		        			html+='<tr>';
		        			html+='<td>'+(i+1)+'</td><td>'+res.data[i].couponName+'</td><td>'+res.data[i].bisName+'</td><td>'+checkStatus+'</td>'
		        			html+='<td>'+res.data[i].writeoffTime+'</td><td>'+res.data[i].couponCode+'</td><td>'+res.data[i].mobile+'</td>';
	    					html+='</tr>'
