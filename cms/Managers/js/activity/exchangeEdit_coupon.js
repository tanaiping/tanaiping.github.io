layui.use(['jquery','element','layer','form','laydate'], function(){
    ...
    function editExchange(activityNo){
    	$.ajax({
          type: "post",
          url: basePath+'/sys/activity/getActivityExchangeDetail?activityNo='+activityNo,
          contentType: "application/json",
          dataType: 'json',
          // data:JSON.stringify(params),
          beforeSend: function (xhr) {
          ...
          success: function(data) {
            ...
            if(data.resultCode==0){
              ...
              var html='';
              html+='<tr attrid="'+res.goodsSkuList[0].attrId+'" unionkey="'+res.goodsSkuList[0].unionKey+'" skuid="'+res.goodsSkuList[0].skuId+'" specid="'+res.goodsSkuList[0].specId+'" activityid="'+res.goodsSkuList[0].activityId+'"><td>1</td><td class="activityName">'+res.goodsName+'</td>'+
                  '<td><img src="'+res.thumbPic+'" alt="" style="height: 60px;"></td>'
	              var price ="";
              ...
              $(".excPrice").val((res.goodsSkuList[0].excPrice)/100);
              $(".excPoint").val(res.goodsSkuList[0].excPoint);
    ...
    function saveExchangeChange(obj,goodsType,type){
    	...
    	$("#exchangeData tr").each(function(){
	    	...
	    	var excPrice = $(".excPrice").val()*100;
	    	var excPoint = $(".excPoint").val();
