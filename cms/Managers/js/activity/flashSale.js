layui.use(['jquery','element','layer','form','laydate','laypage'], function(){
    ...
    //监听搜索
    form.on('submit(searcheSale)', function(data){
      initSaleList(1,true);
      return false;
    });
    //监听搜索
    form.on('submit(exportSale)', function(data){
      exportSaleList(1);
      return false;
    });
    ...
    //修改
    $(document).on('click','.sale-edit',function(){
	  ...
	  if(type==1){
	  	$("#iframe",parent.document.body).attr("src","activity/saleEditCommodity.html?editType=1&rdm="+Math.random()).attr("activityid",activityid);
	  }else{
        $("#iframe",parent.document.body).attr("src","activity/saleEditCoupon.html?editType=1&rdm="+Math.random()).attr("activityid",activityid);
	  }
    });
	
	function initSaleList(curPage,initPage){
		...
		$.ajax({
		    type: "post",
		    url: basePath+'/sys/activity/getActivityPanicBuyList',
		          ...
		          for(var i=0;i<res.length;i++){
		            var act_status = ""
					  ...
					  if(res[i].type==1){//entryType  1 热门推荐   2  兑换专区  3  限时抢购
					  	  detailLink = 'http://cmsfile.wifi8.com/uploads/wifi/AppH5/nfygShop/detailgoods?detailType=goods&entryType=3&'
					  	  //goodsId=XXXXXX&activeId=XXXXXX&checkSkuId=XXXXXX
					  	  //goodsId：商品id，activeId：专区id，checkSkuId：默认选中的sku，entryType：入口类型
					  }else{
					  	  detailLink = 'http://cmsfile.wifi8.com/uploads/wifi/AppH5/nfygShop/detailcoupon?detailType=coupon&entryType=3&'
		              ...
		              html += '<tr index = "'+res[i].index+'" activityid = "'+res[i].activityNo+'" status = "'+res[i].status+'" type = "'+res[i].type+'"><td>'+(i+1)+'</td><td>'+res[i].goodsName+'</td>'+
		                  ...
		                  html += '<td><div>'+res[i].startTime+'</div><div>'+res[i].endTime+'</div></td><td>'+act_status+'</td>'+
							  ...
							  if(res[i].status==0 ||res[i].status==1){
							    ...
							    html += '<button class="layui-btn layui-btn-normal sale-edit">修改</button>'
							  }else if(res[i].status==2){
							  ...
							  // '<button class="layui-btn layui-btn-primary preview-btn">预览</button>'+
		                      '</td></tr>'
		          }
		          $("#saleData").html(html);
	...
	function exportSaleList(curPage){
		var goodsName = $("#goods_name").val();
		var startTime = $("#create_start").val();
		var endTime = $("#create_end").val();
		var status = $("#goods_type").val();
		...
		$.ajax({
		    type: "post",
		    url: basePath+'/sys/activity/exportActivityPanicBuyList',
		    contentType: "application/json",
		    data:JSON.stringify(params),
		    dataType: 'json',
		    beforeSend: function (xhr) {
	...
	function commonOpera(moveType,obj,url){
		  ...
		  $.ajax({
		        ...
		        success: function(data) {
		          if(data.resultCode==0){
//		            window.location.href='flashSale.html?'+Math.random();
					// initSaleList(1,true);
					initSaleList(pages,false);
		          }else if(data.resultCode==3){
		            localStorage.removeItem('role');
		            localStorage.removeItem('token');
		            top.location.href="/cms/login.html?rdm="+Math.random();
		...
		function takeOff(obj){
		  ...
		  var params = {
		    "activityNo":activityid,
		    "status":takeoff,
		    "type":3, // 活动的上下架类型:1-热门推荐,2-兑换专区,3-限时抢购
		  }
		  console.log(params);
		  $.ajax({
		        type: "post",
